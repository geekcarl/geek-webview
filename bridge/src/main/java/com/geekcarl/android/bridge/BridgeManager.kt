...
class BridgeManager(
    ...
    override fun callFunc(
        ...
        postRun {
            val jsCommand = BridgeUtil.generateJavascriptFunctionStr(function, *values)
    ...
    private fun <T> doSend(
        ...
        val request = RequestMessage().apply {
            ...
            this.callbackId = responseCallback?.let {
                val callbackId = BridgeUtil.generateCallbackId(++mUniqueId)
    ...
    init {
        mWebView.addJavascriptInterface(BridgeInterface(this), BridgeUtil.ANDROID_INTERFACE)
